...
export class WidgetSlotEditorComponent {
  ...
  readonly missing = computed(() => {
    ...
    return widget.source === 'sensor' && !(this.hardware()?.sensors ?? []).some(s => s.id === widget.sensorId && s.name === widget.sensorName);
  });
  ...
  key(widget: WidgetConfig) { return widget.source === 'sensor' ? JSON.stringify(['sensor', widget.sensorId, widget.sensorName]) : widget.source === 'metric' ? JSON.stringify(['metric', widget.metricId]) : 'none'; }
