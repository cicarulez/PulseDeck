...
export class WidgetsComponent {
  ...
  readonly slots = computed(() => this.layout() === 'compact' ? this.catalog().slots : this.catalog().slots.slice(0, 8));
  ...
  positionName(index: number, name: string) { return this.layout() === 'compact' ? `Riga ${Math.floor(index / 4) + 1} · Colonna ${index % 4 + 1}` : name; }
